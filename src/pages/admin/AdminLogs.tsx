...
const AdminLogs = () => {
  ...
  // Manter a referência ao hook original para SMS
  const { sendTestSms } = useNotifications();
  ...
  // Enviar email usando o endpoint PHP diretamente
  const handleTestEmail = async () => {
    ...
    // Criar um novo log na UI imediatamente
    ...
    const newLog: NotificationLog = {
      ...
      message: `Enviando email de teste para ${testEmail}`,
      status: 'pending',
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
    
    // Adicionar o novo log à lista
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          // Atualizar o status do log na UI imediatamente
          ...
          // Carregar logs do servidor após breve delay para garantir que todos os logs são actualizados
          setTimeout(() => loadLogs(), 1000);
        } else {
          // Atualizar log para falha
          setLogs(prevLogs => prevLogs.map(log => 
            log.id === emailLogId ? {
              ...log,
              status: 'failed',
              error_message: data.error || 'Falha ao enviar email',
              updated_at: new Date().toISOString()
            } : log
          ));
          ...
          toast({
            title: "❌ Erro",
            description: data.error || "Falha ao enviar email",
            variant: "destructive"
          });
        }
      } else {
        ...
        toast({
          title: "❌ Erro",
          description: "Erro ao enviar email. Verifique o console para detalhes.",
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "❌ Erro",
        description: "Erro ao enviar email de teste. Verifique a conexão.",
        variant: "destructive"
      });
    } finally {
  ...
  const handleTestSms = async () => {
    ...
    // Criar um novo log na UI imediatamente
    const smsLogId = `sms_${Date.now()}`;
    const newLog: NotificationLog = {
      id: smsLogId,
      user_id: 'system',
      notification_type: 'sms',
      recipient: phoneWithPrefix,
      subject: 'SMS de Teste',
      message: `Enviando SMS de teste para ${phoneWithPrefix}`,
      status: 'pending',
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "✅ Sucesso",
          description: `SMS de teste enviado para ${phoneWithPrefix}`
        });
        ...
        // Carregar logs do servidor após breve delay
        setTimeout(() => loadLogs(), 1000);
      } else {
        // Atualizar log para falha
        setLogs(prevLogs => prevLogs.map(log => 
